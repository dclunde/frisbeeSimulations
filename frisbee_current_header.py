...
omega_y   = 15        # Initial speed of rotation in y direction (rot/s)
...
CLA  = 1.23        # Lift coefficient dependent on alpha
...
velocity_and_wind    = [[0,0,0]]*int(total_t/delta_t)
...
def edit_initial_conditions():
    global x,y,z,vx,vy,vz,omega_x,omega_y,omega_z,alpha,phi,alpha_0,wind
    ff = open('frisbee_input.txt','r')
    reset_mat = [0,1,0,14,3,0,0,10,0,8,0,0,0,0]
...
Moments_analysis_file = open('output/Moments_analysis.csv','w')
Moments_analysis_file.write("i value,R,N,M,D_omegax,D_omegay,D_omegaz,delta_omega_x,delta_omega_y,delta_omega_z,alpha,COS,SIN \n")
Moments_analysis_file.close()